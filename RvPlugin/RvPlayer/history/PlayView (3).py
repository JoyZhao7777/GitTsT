...
class PlayView( QDockWidget ):
            ...
            if self.m_combobox_type.isChecked():
                ...
                for shot_name in t_all_shot_name:
                    for task_asset in t_Task_asset_list:
                        shotTask = self.get_shotTask_from_TasknameAnd_shotName(task_asset[0], shot_name)
                        movpath = self.get_mov_path(shotTask, task_asset[-1])
                        if not movpath:
                            continue 
                        Shot = shot_name
                        PathList = [movpath]
                ...
                for count in range( self.m_treewidget.topLevelItemCount() ):
                    ...
                    for i in range( item.childCount() ):
                        ...
                        if cliendItem.checkState( 0 ) == Qt.Checked:
                            ...
                            shotTask = self.get_shotTask_from_TasknameAnd_shotName(task_name,shot_name)
                            #shotTask = self.session.query('Task where name is "{0}" and parent.parent.parent.name is "{1}"'.format(task_name,shot_name)).one()
                            #获取视频路径
                            movpath = self.get_mov_path(shotTask,asset_name)
                            if not movpath:
                                continue 
                            PathList = [movpath] 
    ...
    def get_mov_path(self,shotTask,asset_name = 'ok'):
        movpath = ''
        ...
        for taskAssetVersionC in taskAssetVersion[0]['components']:
            if taskAssetVersionC['file_type'] == ".ma":
                ...
                movpath = self.get_mov_outpath(mov_Path, movName)
                if movpath:
                    return movpath
    ...
    def get_shotTask_from_TasknameAnd_shotName(self,task_name, shot_name):
        shotTask = self.session.query('Task where name is "{0}" and parent.parent.parent.name is "{1}"'.format(task_name, shot_name)).one()
        return shotTask


    def get_mov_outpath(self,movPath,movName):
        if movName == "main":
            movName = ''
